...
// ฟังก์ชันเพิ่มข้อมูลลงใน Google Sheet
...
// ฟังก์ชันอัปเดตข้อมูลใน Google Sheet
...
export async function createDriveFile(filename, mimeType, content, folderId = process.env.GOOGLE_DRIVE_FOLDER_ID) {
  try {
...
// เพิ่มฟังก์ชันนี้ใน googleSheets.js
export async function checkFolderExists(folderId) {
  try {
    ...
    return response.data && response.data.mimeType === 'application/vnd.google-apps.folder';
  } catch (error) {
...
// ปรับปรุงฟังก์ชัน uploadFileToDrive
export async function uploadFileToDrive(fileBuffer, filename, mimeType, folderId = null) {
  try {
    const auth = await getAuthClient();
    const drive = google.drive({ version: 'v3', auth });
    ...
    if (folderId) {
      try {
        ...
        if (!folderExists) {
          console.log(`Folder ID ${folderId} does not exist. Creating a new folder...`);
